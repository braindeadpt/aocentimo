...
/**
 * Wordmark BRUTO — conceito «O Nível»: o U é um recipiente cheio a ~62 %,
 * o que fica do custo do trabalho num salário médio.
 * Texto real (Archivo expandido) — nada de letras posicionadas à mão em
 * SVG, que partiam se a fonte ainda não tivesse carregado. O nível do U
 * é CSS puro (gradiente recortado pela letra), escala com a fonte e
 * adapta-se ao tema pelos tokens.
...
export function Logo({ variant = "ink", className }: LogoProps) {
  return (
    ...
    >
      BR<span className="logo-u">U</span>TO
    </span>
  );
}
